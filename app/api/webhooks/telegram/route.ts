...
export async function POST(req: NextRequest) {
  try {
    ...
    // Find Telegram channel by bot ID stored in phoneNumberId
    // Telegram sends to ONE bot — find the channel whose bot received this update
    // We need to match by bot token — check all Telegram channels
    const telegramChannels = await prisma.channel.findMany({
    ...
    // Use the first connected Telegram channel (single-bot setup)
    // For multi-bot: would need to verify the token via getMe, but that's extra cost
    const channel = telegramChannels[0];
